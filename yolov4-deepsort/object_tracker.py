...
twclient = c(TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN)
...
path = "./data/video/parkingvideo.webm"
# 2020-03-19-08-00-00_scene_0055_BB_write.avi
    ...
    while True:
        ...
        for track in tracker.tracks:
            if not track.is_confirmed():
                ...
                dtime[track.track_id] = datetime.now()
            ...
            if int(dwell_time[track.track_id]) > limit_fine:
                cv2.putText(
                    ...
                    class_name
                    + "-"
                    + str(list_park[track.track_id])
                    + "-"
                    + str(int(dwell_time[track.track_id])),
                ...
                if track.track_id in dwell_unauth:
                    ...
                    time1 = datetime.now()
                    time1 = time1.strftime("%d-%m-%Y %H:%M")
                    vehicle = str(list_park[track.track_id])
                    print(time1, vehicle)
                    # print(track.track_id)
                    try:
                        query1 = part_form.find_one({"vehicle": vehicle})
                        query = {
                        ...
                        ticket.insert_one(query)
                        print("Successfully stored in the db")
                        try:
                            number = "+91{}".format(query1["phone"])
                            number = number[:-2]
                            message = twclient.messages.create(
                                to=number,
                                messaging_service_sid="MG46df0228e275d7eb0652617df912e468",
                                body="Hey! We noticed that you have parked your vehicle in an unauthorized space. Please remove it immediately.",
                            )
                            print(message.sid)
                        except TwilioRestException as e:
                            print(e)
                    except:
                        print("Database Connection Error. Aborting...")
                        break
